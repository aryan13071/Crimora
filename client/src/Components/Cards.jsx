...
const Cards = ({ task, img }) => {
  ...
  const handleClick = () => {
    // map each task to its route
    if (task === "Track Your Path") navigate("/track");
    else if (task === "Report a Crime") navigate("/report");
    else if (task === "View Analytics") navigate("/analyse");
    else if (task === "Your Contribution") navigate("/contribute");
  };
